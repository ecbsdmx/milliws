...
Router.route('/jobs', {
  ...
  data: function() { if(this.ready()) {return Jobs.find();} }
});
...
Router.route('/:fromCount?', {
  name: 'eventsList',
  waitOn: function() {
    Meteor.call("messageLogDebug", "Events route subscriptions (waitOn)", "router");
    var l = Session.get("EventsFromCount") || 0;
    var filterOptions = Session.get("eventsFilter") || {};
    return [
      Meteor.subscribe('events', l, getSortOptions(), filterOptions),
      Meteor.subscribe('eventStats'),
      Meteor.subscribe('eventsCount', filterOptions)
    ];
  },
  data: function() {
    if(this.ready()) { //check https://github.com/iron-meteor/iron-router/issues/1193
      Meteor.call("messageLogDebug", "Events route data rdy", "router");
