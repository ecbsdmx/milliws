...
Router.route('/reports', {
  name: 'reportsList', 
  waitOn: function() { 
    return Meteor.subscribe('eventsStatPerJob'); 
  }, 
  data: function() { return EventsStatPerJob.find(); }
});

Router.route('/jobs', {
  name: 'jobsList', 
  template: "jobsList", 
  waitOn: function() { return [Meteor.subscribe('jobs'), Meteor.subscribe('eventsStatPerJob')]; },
...
Router.route('/jobs/recycle', {
  name: 'jobsRecycle', 
  template: "jobsList", 
  waitOn: function() { return [Meteor.subscribe('recycledJobs'), Meteor.subscribe('eventsStatPerJob')]; }, 
  data: function() { return Jobs.find(); }
});
...
Router.route('/events/:fromCount?', {
  ...
  waitOn: function() { 
    ...
    return [Meteor.subscribe('events', l), Meteor.subscribe('eventsStatPerJob')]; 
  }, 
  data: function() { 
    return Events.find(); 
  }
