...
if (Meteor.isServer) {
  Meteor.methods({
    ...
    compileDailyAgg: function(breakdownType, selectedJobs, date, tzOffset) {
      ...
      var periodEnd = date;
      var periodStart = new Date(periodEnd.getFullYear() -1, periodEnd.getMonth(), periodEnd.getDate());
      switch(breakdownType) {
        case "rtBreakdown":
          all = Events.aggregate(
            [
              {$match: {jobId : {$in: selectedJobs}, etime: {$gte: periodStart}}},
              {$group: {
