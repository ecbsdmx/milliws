/**
 * Jobs represent the monitoring queries run by MilliWS on a regular basis.
 *
...
Meteor.methods({
  ...
  jobToggleActiveFlag: function(jobId, flag) {
    checkPermissions();
    check(jobId, String);
    check(flag, Boolean);
    Jobs.update({_id: jobId, owner: this.userId}, {$set:{isActive: flag}});
  }
});
