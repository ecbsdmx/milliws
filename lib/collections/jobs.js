...
var validUrl = /^(https?:\/\/)?([a-z\.-]+)\.([a-z\.]{2,6})([\/\w\.-]*)*\/.*data\/(([a-z0-9_@$\-]+)|(([a-z][a-z0-9_\-]*(\.[a-z][a-z0-9_\-]*)*)(\,[a-z0-9_@$\-]+)(\,(latest|([0-9]+(\.[0-9]+)*)))?))\/?(([a-z0-9_@$\-]+([+][a-z0-9_@$\-]+)*)?([.]([a-z0-9_@$\-]+([+][a-z0-9_@$\-]+)*)?)*)\/?(([a-z][a-z0-9_\-]*(\.[a-z][a-z0-9_\-]*)*\,)?([a-z0-9_@$\-]+))\/?[\?]?(.*)$/gi;

var IsUrl = Match.Where(function(x) {
  check(x, String);
  return -1 < x.search(validUrl);
});
